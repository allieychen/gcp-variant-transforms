...
class VariantCall(object):
  ...
  def __init__(self, name=None, genotype=None, phaseset=None, info=None,
               sample_id=None, file_path=None):
    # type: (str, List[int], str, Dict[str, Any], int, str) -> None
    ...
    Args:
      ...
      info: A map of additional variant call information. The key is specified
        ...
        header FORMAT.
      sample_id: A hash code that uniquely identifies a sample.
      file_path: The file path where the call comes from.
    """
    self.name = name
    self.genotype = genotype or []
    self.phaseset = phaseset
    self.info = info or {}
    self.call_id = sample_id
    self.file_path = file_path

  def __eq__(self, other):
    return (
            (self.name, self.genotype, self.phaseset, self.info, self.call_id) ==
...
class PyVcfParser(VcfParser):
  ...
  def _get_variant_calls(self, record, formats):
    ...
    for sample in record.samples:
      ...
      if self._file_hash:
        call.call_id = generate_int64_hash_code(self._file_hash + call.name)
        call.file_path = self._file_name
        call.file_id = generate_int64_hash_code(self._file_hash)
      calls.append(call)
    return calls
...
def generate_int64_hash_code(file_hash, sample_name):
  # type: (str, str) -> int
  """Generates a int64 hash code based on file_hash + sample_name.

  The hash code generated by CityHash64 is unsigned. The int64 field in BQ is
  signed. -sys.maxint - 1 is used to create a 1 to 1 mapping from unsigned int
  to signed int.
  """
  return generate_int64_hash_code(file_hash + sample_name)


def generate_int64_hash_code(s):
  # type: (str, str) -> int
  """Generates a int64 hash code based on file_hash + sample_name.
  ...
  to signed int.
  """
  return cityhash.CityHash64(s) - sys.maxint - 1